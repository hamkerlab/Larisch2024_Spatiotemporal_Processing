...
def shuffleW(weights):
    post,pre = np.shape(weights)
    weights = np.reshape(weights,post*pre)
...
def loadWeights():
    ...
    w = np.loadtxt('Input_network/InhibW.txt')
    projLGN_Inh.w = shuffleW(w) 
    w = np.loadtxt('Input_network/V1toIN.txt')
    projV1_Inhib.w = shuffleW(w) 
    w = np.loadtxt('Input_network/INtoV1.txt')
    projInhib_V1.w = shuffleW(w) 
    w = np.loadtxt('Input_network/INLat.txt')
    projInhib_Lat.w = shuffleW(w) 
