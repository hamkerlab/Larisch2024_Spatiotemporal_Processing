...
#####
# Present moving gratings to redo the Movshon et al. (1986) Task
# Show two sinus grating with a orthogonal moving direction
#####
...
def createInput(degree,totalT,presentT,spat_speed,amplitude=0.3,s_f=1.12):
    ...
    for i in range(n_img_total): 
        ...
        p#lt.colorbar()
        p#lt.savefig('Output/direct_gratingSinus/inpt_d1_degree_%i_spatFreq_%i_sF_%f_%i.png'%(degree,spat_speed,s_f,i))
        p#lt.close()

    return(img_list)        

def createPlaid(degree,totalT,presentT, rotate,spat_speed, amplitude=0.3,s_f=1.12):
    ## create a  plaid with 120 degree between them (see Movshon 198x)
    ## use the "old" degree value as the middle value ( so +- 60 degree) 
    p_degree = (degree+rotate)%360
    m_degree = (degree-rotate)%360
    #print(p_degree,degree,m_degree)
    #spat_speed -> speed of grating
    edge = int(s_input*0.5)
    image_size = s_input+edge  #h_LGN +2
    #s_f = 1.12#0.048*s_input#0.3#
    ...
    for i in range(n_img_total): 
        parameters[6] = i*step# x_center position
        ## first sinus grating
        grat_1 = grating.createGaborMatrix(parameters,image_size,image_size)
        grat_1 = ndimage.rotate(grat_1,p_degree,reshape=False,mode='wrap')
        ## second sinus grating
        grat_2 = grating.createGaborMatrix(parameters,image_size,image_size)
        grat_2 = ndimage.rotate(grat_2,m_degree,reshape=False,mode='wrap')
        ...
        #plt.imshow(img_list[i],cmap='gray',interpolation='none',vmin=0.0,vmax=0.5)
        #plt.colorbar()
        #plt.savefig('Output/direct_gratingSinus/plaid_d1_degree_%i_spatFreq_%i_sF_%f_%i.png'%(degree,spat_speed,s_f,i))
        #plt.close()
...
def main():
    ...
    if not os.path.exists('Output/direct_gratingSinus_Movshon/'):
        os.mkdir('Output/direct_gratingSinus_Movshon/')
    ...
    rec_I1_spikes = np.zeros((n_degrees,repeats,n_E1, totalT))
            ...
            for tF in range(n_sf_steps):
                for r in range(repeats):
                    ...
                    for d in l_degrees:
                        ...
                        inp_list = createPlaid(d,totalT,presentT, rotate,spat_speed[sf],amp_start,sf_steps[tF])
                ...
                np.save('./work/Movshon_SpikeCount_LGN_rotate%i_amp%i_spatF%i_tempF%i'%(rotate,a,sf,tF),rec_LGN, allow_pickle=False)
                np.save('./work/Movshon_SpikeCount_E1_rotate%i_amp%i_spatF%i_tempF%i'%(rotate,a,sf,tF),rec_E1, allow_pickle=False)
                np.save('./work/Movshon_SpikeTimes_E1_rotate%i_amp%i_spatF%i_tempF%i'%(rotate,a,sf,tF),rec_E1_spikes, allow_pickle=False)
                np.save('./work/Movshon_SpikeCount_I1_rotate%i_amp%i_spatF%i_tempF%i'%(rotate,a,sf,tF),rec_IL1, allow_pickle=False)
                np.save('./work/Movshon_SpikeTimes_I1_rotate%i_amp%i_spatF%i_tempF%i'%(rotate,a,sf,tF),rec_I1_spikes, allow_pickle=False)
    ...
    pbar.close()
    np.savetxt('./work/Movshon_parameter.txt',[totalT,1,presentT])
    pickle.dump(sim_parameter,open('./work/Movshon_parameter.p',"wb"))
