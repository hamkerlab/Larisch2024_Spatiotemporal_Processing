...
def plot(list_STA, list_STRF, name):
    ...
    fig.savefig('./Output/STRF/STRF_'+name+'/STAEX_NOISE_Test_2.jpg',bbox_inches='tight', pad_inches = 0.1,dpi=300)
    ...
    for i in range(n_cells):
        ...
        data -=np.mean(data)
        plt.subplot(x,y,i+1)
        im = plt.imshow(list_STRF[i],cmap=plt.get_cmap('RdBu',7),aspect='auto',interpolation='none',vmin=-np.max(np.abs(data)),vmax=np.max(np.abs(data)))
        if i%x==0:
            plt.ylabel('t', fontsize=12)
            plt.xlabel('x', fontsize=12)
            plt.xticks([0,w],[0,w],fontsize=8)
            plt.yticks([0,t],[0,t],fontsize=8)
        else:
            plt.xticks([])
            plt.yticks([])
        #plt.axis('equal')
    #plt.axis('equal')
    plt.subplots_adjust(hspace=0.25,wspace=0.25)
    fig.savefig('./Output/STRF/STRF_'+name+'/STRF_NOISE_Test_2.jpg',bbox_inches='tight', pad_inches = 0.1,dpi=300)
