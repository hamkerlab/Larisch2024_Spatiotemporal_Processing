...
def startAnalye2d():
    ...
    for i in tqdm(range(n_cells),ncols=80):
        ...
        mse_list[i] = np.sqrt(np.mean((half_q[t_c-z:t_c+1,z_x:] - np.flip(half_q[t_c:t_c+z+1,z_x:],axis=0))**2))

        """
        plt.figure()
        plt.subplot(2,1,1)
        plt.imshow(half_q[t_c-z:t_c+1,z_x:], vmin = 0, vmax = np.max(half_q))
        plt.colorbar()
        plt.subplot(2,1,2)
        plt.imshow(np.flip(half_q[t_c:t_c+z+1,z_x:],axis=0), vmin = 0, vmax = np.max(half_q))
        plt.colorbar()
    ...
    plt.figure()
    plt.hist(mse_list, range=(0,0.16),bins=9)
    ...
    n_bins=10
    bins = np.linspace(0,0.16,n_bins)
    print(bins)
