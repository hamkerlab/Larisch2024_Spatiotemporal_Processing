...
def createInput(degree,totalT,presentT,spat_Freq,amplitude=0.3,s_f=1.12):
...
def main():
    ...
    presentT = 20#int(1000/spat_Freq)
    ...
    n_sf_steps = 10
    sf_steps = np.linspace(0.4,1.8,n_sf_steps)
    ...
    rec_LGN = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,n_LGN))

    rec_E1 = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,n_E1))
    rec_membPotEx = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,totalT,n_E1))
    rec_gExcEx = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,totalT,n_E1))
    rec_gInhEx = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,totalT,n_E1))

    rec_IL1 = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,n_I1))
    rec_membPotIL1 = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,totalT,n_I1))
    rec_gExIL1 = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,totalT,n_I1))
    rec_gInIL1 = np.zeros((n_amplitudes,n_sf_steps,n_degrees,repeats,totalT,n_I1))
        ...
        for sf in range(n_sf_steps):
            for r in range(repeats):
                ...
                for d in l_degrees:
                    ...
                    inp_list = createInput(d,totalT,presentT,spat_Freq,amp_start+((a+1)*amplitude_step),sf_steps[sf])
                    ...
                    for n in range(n_LGN):
                        spk = spk_LGN[n]
                        rec_LGN[a,sf,int(d//s_deg),r,n] = (len(spk)/totalT)*1000

                    for n in range(n_E1):
                        spk = spk_E1[n]
                        rec_E1[a,sf,int(d//s_deg),r,n] = (len(spk)/totalT)*1000
                        ...
                        if n < int(n_E1/4):
                            spk = spk_I1[n]
                            rec_IL1[a,sf,int(d//s_deg),r,n] = (len(spk)/totalT)*1000

                            #spk = spk_I2[n]
                            #rec_IL2[a,sf,int(d//s_deg),r,n] = (len(spk)/totalT)*1000

                    rec_membPotEx[a,sf,int(d//s_deg),r] = vmEx#[0:totalT]
                    rec_gExcEx[a,sf,int(d//s_deg),r] = gExcEx#[0:totalT]
                    rec_gInhEx[a,sf,int(d//s_deg),r] = gInhEx#[0:totalT]

                    rec_membPotIL1[a,sf,int(d//s_deg),r] = vmIL1#[0:totalT]

                    rec_gExIL1[a,sf,int(d//s_deg),r] = gExIL1#[0:totalT]
                    rec_gInIL1[a,sf,int(d//s_deg),r] = gInIL1#[0:totalT]
