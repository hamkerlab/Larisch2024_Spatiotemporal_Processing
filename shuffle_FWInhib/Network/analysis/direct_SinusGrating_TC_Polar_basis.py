...
def moreDSI(preff_E1, path):
    ...
    pref_Direction = np.zeros(n_cellsE1)
    ...
    for c in range(n_cellsE1):
        ...
        ##NOTE: We operate on the grating orientation, not direction (!) -> for each cell orientation, there exists two sinus graing orientations/directions !
        pref_D1 = pref_O
        ...
        pref_Direction[c] = ((pref_arg * step_degree)-90)%360
    ...
    np.save('./work/prefD_basis_'+path,pref_Direction)
    ...
    a,b = np.histogram(pref_Direction, bins=np.arange(0,360+bin_size,bin_size))

    plt.figure()
    ax = plt.subplot(121,projection='polar')
    bars = ax.bar(bins/180*np.pi, a, width=0.2, bottom=0.1)
    ax = plt.subplot(122)
    plt.hist(pref_Direction,bins=10)
    plt.savefig('./Output/DSI_TC_basis/'+path+'/DSI_hist/hist_PreffD.png')
