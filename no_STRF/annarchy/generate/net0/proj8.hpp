...
/////////////////////////////////////////////////////////////////////////////
// proj8: E1 -> I1 with target Exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct8 : LILInvMatrix<int, int> {
    ProjStruct8() : LILInvMatrix<int, int>( 81, 324) {
    }
...
int nb_post; double sum;
        ...
        if (_transmission && pop6._active){
            ...
            for(int _idx_j = 0; _idx_j < pop5.spiked.size(); _idx_j++){
                // Rank of the presynaptic neuron
                int rk_j = pop5.spiked[_idx_j];
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct8::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct8::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct8::get_local_attribute_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
    ...
    void set_local_attribute_all_double(std::string name, std::vector<std::vector<double>> value) {
        ...
        std::cout << "ProjStruct8::set_local_attribute_all_double(name = " << name << ", min(" << name << ")=" <<std::to_string(min_value) << ", max("<<name<<")="<<std::to_string(max_value)<< ")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct8::set_local_attribute_row_double(name = "<<name<<", rk_post = " << rk_post << ", min("<<name<<")="<<std::to_string(*std::min_element(value.begin(), value.end())) << ", max("<<name<<")="<<std::to_string(*std::max_element(value.begin(), value.end()))<< ")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct8::set_local_attribute_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<", value = " << std::to_string(value) << ")" << std::endl;
    #endif
