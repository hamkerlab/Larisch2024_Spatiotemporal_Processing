...
/////////////////////////////////////////////////////////////////////////////
// proj6: pop4 -> E1 with target Exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct6 : LILInvMatrix<int, int> {
    ProjStruct6() : LILInvMatrix<int, int>( 324, 648) {
    }
...
int nb_post; double sum;
        ...
        if (_transmission && pop5._active){
            ...
            for (int idx_spike=0; idx_spike < pop4.spiked.size(); idx_spike++) {

                // Get the rank of the pre-synaptic neuron which spiked
                int rk_pre = pop4.spiked[idx_spike];
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct6::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct6::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct6::get_local_attribute_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
    ...
    void set_local_attribute_all_double(std::string name, std::vector<std::vector<double>> value) {
        ...
        std::cout << "ProjStruct6::set_local_attribute_all_double(name = " << name << ", min(" << name << ")=" <<std::to_string(min_value) << ", max("<<name<<")="<<std::to_string(max_value)<< ")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct6::set_local_attribute_row_double(name = "<<name<<", rk_post = " << rk_post << ", min("<<name<<")="<<std::to_string(*std::min_element(value.begin(), value.end())) << ", max("<<name<<")="<<std::to_string(*std::max_element(value.begin(), value.end()))<< ")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct6::set_local_attribute_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<", value = " << std::to_string(value) << ")" << std::endl;
    #endif
