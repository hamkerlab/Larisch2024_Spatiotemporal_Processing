...
/////////////////////////////////////////////////////////////////////////////
// proj1: pop0 -> pop2 with target exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct1 : LILMatrix<int, int> {
    ProjStruct1() : LILMatrix<int, int>( 484, 2304) {
    }
        ...
        if (_transmission && pop2._active) {
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    sum += pop0._delayed_r[delay[i][j]-1][pre_rank[i][j]]*w[i][j] ;
                }
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct1::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct1::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct1::get_local_attribute_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
    ...
    void set_local_attribute_all_double(std::string name, std::vector<std::vector<double>> value) {
        ...
        std::cout << "ProjStruct1::set_local_attribute_all_double(name = " << name << ", min(" << name << ")=" <<std::to_string(min_value) << ", max("<<name<<")="<<std::to_string(max_value)<< ")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct1::set_local_attribute_row_double(name = "<<name<<", rk_post = " << rk_post << ", min("<<name<<")="<<std::to_string(*std::min_element(value.begin(), value.end())) << ", max("<<name<<")="<<std::to_string(*std::max_element(value.begin(), value.end()))<< ")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct1::set_local_attribute_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<", value = " << std::to_string(value) << ")" << std::endl;
    #endif
