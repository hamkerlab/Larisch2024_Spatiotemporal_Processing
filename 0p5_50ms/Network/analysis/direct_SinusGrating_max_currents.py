...
def moreDSI():
    ...
    ## load the STRFs
    strf_E1 = np.load('./work/STRF_data_E1.npy')
    print(np.shape(strf_E1))
    dsi_arg = np.argsort(dsi_kim[0][0:16])
    plt.figure()
    for i in range(16):
        plt.subplot(4,4,i+1)
        plt.title(np.round(dsi_kim[0][dsi_arg[i]],3))
        plt.imshow(np.mean(strf_E1[dsi_arg[i]],axis=-1),cmap=plt.get_cmap('gray',7), interpolation='none', aspect='auto')
        plt.axis('off')
    plt.savefig('./Output/DSI/STRF')
    plt.close()
    ...
    plt.ylabel('# Cells')
    plt.savefig('./Output/DSI/hist_Correlation')
    ...
    plt.xlabel('Correlation gEx - gInh')
    plt.ylabel('DSI')
    plt.savefig('./Output/DSI/hist_Correlation_scatter')
    ...
    plt.ylabel('DSI')
    plt.savefig('./Output/DSI/delta_T_dsi')
    ...
    plt.savefig('./Output/DSI/violon_deltaT')
    plt.close()
    ...
    plt.savefig('./Output/DSI/violon_deltaT_abs')
    plt.close()
