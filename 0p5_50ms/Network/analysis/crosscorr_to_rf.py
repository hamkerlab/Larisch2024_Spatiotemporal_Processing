...
def analyze():
    ...
    idx_midhighDSI = np.where((dsi_kim > 0.6) & (dsi_kim < 0.8))[0]

    idx_lowDSI = np.where(dsi_kim<0.2)[0]

    gabor_e_high = np.copy(gabor_e[idx_highDSI])
    ori_ex = gabor_e_high[:,1]/np.pi*180.0
    phase_ex = gabor_e_high[:,5]
    xc_ex = gabor_e_high[:,6]
    yc_ex = gabor_e_high[:,7]

    preff_D_highDSI = preff_E1[0,idx_highDSI,2]
    temp_pref = temp_E_I[idx_highDSI]

    ### just let look only on cells with a high DSI
    spk_E, gEx_E, gIn_E, spk_I = loadTime_Data(preff_E1, idx_highDSI)
    ...
    for c in tqdm(range(len(spk_E)), ncols=80):
        ...
        mean_aprox_gInh_pref = np.mean(aprox_gInh_pref,axis=0)
        mean_aprox_gInh_null = np.mean(aprox_gInh_null,axis=0)

        mean_gEx_E_pref = np.mean(gEx_E_pref,axis=0)
        mean_gEx_E_null = np.mean(gEx_E_null,axis=0)
    
        #print(np.mean(mean_gEx_E_pref))


        for i in range(n_inh):
            if np.max(mean_aprox_gInh_pref[i]) > 0:
                crossCorr_p = calcCrossCorr(mean_gEx_E_pref/np.max(mean_gEx_E_pref), mean_aprox_gInh_pref[i]/np.max(mean_aprox_gInh_pref[i]), corr_w)        
                crossCorr_pref[c,i] = crossCorr_p
            crossCorr_pref_maxDT[c,i] = getDeltaT(crossCorr_p,corr_w)

            if np.max(mean_aprox_gInh_null[i]) > 0:
    ...
    np.save('./work/crossCorr_pref_currents_singleInh', crossCorr_pref)
    np.save('./work/maxDT_pref_currents_singleInh', crossCorr_pref_maxDT)
    np.save('./work/maxDT_null_currents_singleInh', crossCorr_null_maxDT)
    np.save('./work/crossCorr_null_currents_singleInh', crossCorr_null)
...
def plotAllCells():
    ...
    crossCorr_pref = np.load('./work/crossCorr_pref_currents_singleInh.npy')
    crossCorr_pref_maxDT = np.load('./work/maxDT_pref_currents_singleInh.npy')

    crossCorr_null = np.load('./work/crossCorr_null_currents_singleInh.npy')
    crossCorr_null_maxDT = np.load('./work/maxDT_null_currents_singleInh.npy')
    ...
    for c in tqdm(range(len(idx_highDSI)), ncols=80):
        ...
        plt.colorbar()
        plt.scatter(xc_ex[c],yc_ex[c], c = 'red')
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_estDeltaT_Map.png'%(c))
        plt.close()
        ...
        plt.xlabel(r'$\Delta T$')
        plt.ylabel('euclidian distance')
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_DeltaTtoDistance'%(c))
        plt.close()
        ...
        plt.xlabel(r'$\Delta T$')
        plt.ylabel('orientation distance')
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_DeltaTtoOrientation'%(c))
        plt.close()
        ...
        plt.xlabel(r'$\Delta T$')
        plt.ylabel('phase difference')
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_DeltaTtoPhase'%(c))
        plt.close()
        ...
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_testCrossCorr_pref.png'%(c))
        plt.close()
        ...
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_testCrossCorr_null.png'%(c))
        plt.close()
        ...
        plt.xlabel(r'$\Delta T$')
        plt.ylabel('cos')
        plt.savefig('Output/DSI_TC/Inhib_RF/cells/cell_%i_TMtoDeltaT.png'%(c))
        plt.close()
...
def plotBinned():
    ...
    plt.xlabel(r'$\Delta T$')
    plt.ylabel('euclidian distance')
    plt.savefig('Output/DSI_TC/Inhib_RF/DeltaTtoDistance_allC')
    plt.close()
    ...
    plt.xlabel(r'$\Delta T$')
    plt.ylabel('orientation distance')
    plt.savefig('Output/DSI_TC/Inhib_RF/DeltaTtoOrientation_allC')
    plt.close()
    ...
    plt.xlabel(r'$\Delta T$')
    plt.ylabel('phase difference')
    plt.savefig('Output/DSI_TC/Inhib_RF/DeltaTtoPhase_allC')
    plt.close()
    ...
    plt.xlabel(r'$\Delta T$')
    plt.ylabel('cos')
    plt.savefig('Output/DSI_TC/Inhib_RF/TMtoDeltaT_allC.png')
    plt.close()    
    ...
    partial_corr_sp_pref_1 = np.zeros(len(idx_highDSI))
    partial_corr_or_pref_1 = np.zeros(len(idx_highDSI))
    partial_corr_sp_null_1 = np.zeros(len(idx_highDSI))
    partial_corr_or_null_1 = np.zeros(len(idx_highDSI))

    ## between spatial difference and phase difference
    partial_corr_sp_pref_2 = np.zeros(len(idx_highDSI))
    partial_corr_ph_pref_2 = np.zeros(len(idx_highDSI))
    partial_corr_sp_null_2 = np.zeros(len(idx_highDSI))
    partial_corr_ph_null_2 = np.zeros(len(idx_highDSI))

    ## between phase difference and orientation difference
    partial_corr_or_pref_3 = np.zeros(len(idx_highDSI))
    partial_corr_ph_pref_3 = np.zeros(len(idx_highDSI))
    partial_corr_or_null_3 = np.zeros(len(idx_highDSI))
    partial_corr_ph_null_3 = np.zeros(len(idx_highDSI))


    for c in range(len(idx_highDSI)):
    ...
    plt.savefig('Output/DSI_TC/Inhib_RF/Partial_Correlation_space_orientation.png',dpi=300,bbox_inches='tight')
    plt.close()
    ####
    ...
    maxDT_TM_bins_pref = np.zeros((len(idx_highDSI), n_bins))
    maxDT_TM_bins_null = np.zeros((len(idx_highDSI), n_bins))
    print(tm_bin_bounds)
    for c in range(len(idx_highDSI)): # go over all exc cells
        for b in range(len(tm_bin_bounds)-1): # go through all bounds (except the last one)
            ...
            if len(idx) > 0:
                maxDT_TM_bins_pref[c,b] = np.mean(crossCorr_pref_maxDT[c,idx]) # average maxDT in this bin
                maxDT_TM_bins_null[c,b] = np.mean(crossCorr_null_maxDT[c,idx])

    plt.figure()
    plt.plot(np.mean(maxDT_TM_bins_pref,axis=0),'o--', label='pref')
    plt.plot(np.mean(maxDT_TM_bins_null,axis=0),'o--', label='null')
    plt.legend()
    plt.savefig('Output/DSI_TC/Inhib_RF/TMtoDeltaT_Binned_Test.png')
    ...
    plt.savefig('Output/DSI_TC/Inhib_RF/TMtoDeltaT_Binned_Box.png',dpi=300,bbox_inches='tight')
    ...
    maxDT_ED_bins_pref = np.zeros((len(idx_highDSI), n_bins))
    maxDT_ED_bins_null = np.zeros((len(idx_highDSI), n_bins))
    print(ed_bin_bounds)
    for c in range(len(idx_highDSI)): # go over all exc cells
        for b in range(len(ed_bin_bounds)-1): # go through all bounds (except the last one)
            ...
            if len(idx) > 0:
                maxDT_ED_bins_pref[c,b] = np.mean(crossCorr_pref_maxDT[c,idx]) # average maxDT in this bin
                maxDT_ED_bins_null[c,b] = np.mean(crossCorr_null_maxDT[c,idx])

    plt.figure()
    plt.plot(np.mean(maxDT_ED_bins_pref,axis=0),'o--', label='pref')
    plt.plot(np.mean(maxDT_ED_bins_null,axis=0),'o--', label='null')
    plt.legend()
    plt.savefig('Output/DSI_TC/Inhib_RF/EDtoDeltaT_Binned_Test.png')
    ...
    axs[1].hlines(0,1,n_bins,colors='gray', linestyles='dashed')
    plt.savefig('Output/DSI_TC/Inhib_RF/EDtoDeltaT_Binned_Box.png',dpi=300,bbox_inches='tight')
    ...
    maxDT_OR_bins_pref = np.zeros((len(idx_highDSI), n_bins))
    maxDT_OR_bins_null = np.zeros((len(idx_highDSI), n_bins))
    print(or_bin_bounds)
    for c in range(len(idx_highDSI)): # go over all exc cells
        for b in range(len(or_bin_bounds)-1): # go through all bounds (except the last one)
            ...
            if len(idx) > 0:
                maxDT_OR_bins_pref[c,b] = np.mean(crossCorr_pref_maxDT[c,idx]) # average maxDT in this bin
                maxDT_OR_bins_null[c,b] = np.mean(crossCorr_null_maxDT[c,idx])

    plt.figure()
    plt.plot(np.mean(maxDT_OR_bins_pref,axis=0),'o--', label='pref')
    plt.plot(np.mean(maxDT_OR_bins_null,axis=0),'o--', label='null')
    plt.legend()
    plt.savefig('Output/DSI_TC/Inhib_RF/ORtoDeltaT_Binned_Test.png')
    ...
    axs[1].hlines(0,1,n_bins,colors='gray', linestyles='dashed')
    plt.savefig('Output/DSI_TC/Inhib_RF/ORtoDeltaT_Binned_Box.png',dpi=300,bbox_inches='tight')
    ...
    maxDT_PH_bins_pref = np.zeros((len(idx_highDSI), n_bins))
    maxDT_PH_bins_null = np.zeros((len(idx_highDSI), n_bins))
    ...
    for c in range(len(idx_highDSI)): # go over all exc cells
        for b in range(len(ph_bin_bounds)-1): # go through all bounds (except the last one)
            ...
            if len(idx) > 0:
                maxDT_PH_bins_pref[c,b] = np.mean(crossCorr_pref_maxDT[c,idx]) # average maxDT in this bin
                maxDT_PH_bins_null[c,b] = np.mean(crossCorr_null_maxDT[c,idx])

    plt.figure()
    plt.plot(np.mean(maxDT_PH_bins_pref,axis=0),'o--', label='pref')
    plt.plot(np.mean(maxDT_PH_bins_null,axis=0),'o--', label='null')
    plt.legend()
    plt.savefig('Output/DSI_TC/Inhib_RF/PHtoDeltaT_Binned_Test.png')
    ...
    axs[1].hlines(0,1,n_bins,colors='gray', linestyles='dashed')
    plt.savefig('Output/DSI_TC/Inhib_RF/PHtoDeltaT_Binned_Box.png',dpi=300,bbox_inches='tight')
